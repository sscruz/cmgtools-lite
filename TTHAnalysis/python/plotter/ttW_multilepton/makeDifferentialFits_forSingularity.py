...
if todo == "2":
   ...
   nCores = 5
   queue = "batch"
   submit = '''sbatch -c %d -p %s  --wrap 'cmssw-cc7  --command-to-run "cd {combinerepo} && cmsenv && cd {foldtorun} && ulimit -s unlimited && {command}" ' '''%(nCores, queue)
   print(submit.format(combinerepo=comb,foldtorun=os.getcwd(),command=rf))
   print(submit.format(combinerepo=comb,foldtorun=os.getcwd(),command=rff))
   if pr: 
      os.system(submit.format(combinerepo=comb,foldtorun=os.getcwd(),command=rf))
      os.system(submit.format(combinerepo=comb,foldtorun=os.getcwd(),command=rf))
   #os.system(rf)
   #os.system(rff)
