# this is used to get the matrix element from the reference point!!
...
class globalEFTreWeighting( Module ):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        for card in self.param_cards:
            self.wrappedOutputTree.branch('matrix_reference_%s'%(card.split('/')[-1].replace('param_card_','').replace('.','_')),'F')
            self.wrappedOutputTree.branch('matrix_reference_nohel_%s'%(card.split('/')[-1].replace('param_card_','').replace('.','_')),'F')
    ...
    def analyze(self, event):
        ...
        for mod in self.mods:
            ...
            weights_nohel.append( mod.smatrixhel( final_pdgs, final_parts_i, event.LHE_AlphaS, scale2, -1) ) 

        for i, card in enumerate(self.param_cards):
            self.wrappedOutputTree.fillBranch('matrix_reference_%s'%(card.split('/')[-1].replace('param_card_','')).replace('.','_'), weights[i])
            self.wrappedOutputTree.fillBranch('matrix_reference_nohel_%s'%(card.split('/')[-1].replace('param_card_','')).replace('.','_'), weights_nohel[i])
